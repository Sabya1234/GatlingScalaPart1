...
class ComplexCustomFeeder extends Simulation {
  ...
  val customfeeders = Iterator.continually(Map(
    "gameid" -> idNum.next(), "name" -> ("Game-" + getString(5)), "releaseDate" -> getDate(now, rand), "reviewScore" -> rand.nextInt(80),
    "rating" -> ("Rating-" + getString(4)), "category" -> ("Category-" + getString(4))
  ))

  def authenticate() = {
    exec(http("authenticate").post("/authenticate").body(RawFileBody("Data/cred.json")).asJson
