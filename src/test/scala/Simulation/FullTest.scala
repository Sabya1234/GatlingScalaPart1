...
class FullTest extends Simulation {
  ...
  val csvFeeder = csv("Data/game.csv").random
  ...
  def authenticate: ChainBuilder =
    exec(http("authenticate").post("/authenticate")
      .body(RawFileBody("Data/cred.json")).asJson
  ...
  def getSpecificGame =
    exec(http("get specific game").get("/videogame/${gameid}")
      .check(status.is(200)))
      .pause(1)

  def deleteSpecificGame =
    exec(http("delete any game").delete("/videogame/${gameid}")
      .header("authorization", "Bearer ${jwtToken}")
